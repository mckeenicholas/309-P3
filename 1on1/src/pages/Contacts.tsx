...
const Contacts = () => {
      ...
      try {
        ...
        if (result) {
          const contacts: Contact[] = result.map((contact: RawContact) => ({
            ...
            fullname:
              contact.contactee_info.first_name +
              " " +
              contact.contactee_info.last_name,
  ...
  const deleteContact = async (contactee: string) => {
    try {
      ...
      setContacts((contacts) =>
        contacts.filter(
          (contact) => contact.contactee.toString() !== contactee,
        ),
      );
    } catch (error) {
      console.error(error);
    ...
    <div id="wrapper" className="d-flex">
      <div id="page-content-wrapper">
        <div className="col-md-9">
          ...
          <button
            className="add-contact-btn btn btn-outline-success mt-5"
            onClick={openModal}
          >
            Add New Contact
          </button>

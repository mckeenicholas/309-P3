...
const TimeSlotSelector: React.FC<TimeSlotSelectorProps> = ({
  ...
  const [prioritySelection, setPrioritySelection] = useState<"high" | "low">(
    "high",
  );
  ...
  const toggleSlotSelection = (slotIdentifier: string): void => {
    ...
    const currentPriorityList =
      newSelectedSlots[isHighPriority ? "highPriority" : "lowPriority"];
    const otherPriorityList =
      newSelectedSlots[isHighPriority ? "lowPriority" : "highPriority"];
    ...
    if (currentIndex > -1) {
      newSelectedSlots[isHighPriority ? "highPriority" : "lowPriority"] =
        currentPriorityList.filter((item) => item !== slotIdentifier);
      ...
      newSelectedSlots[isHighPriority ? "lowPriority" : "highPriority"] =
        otherPriorityList.filter((item) => item !== slotIdentifier);
    } else {
      newSelectedSlots[isHighPriority ? "highPriority" : "lowPriority"] = [
        ...currentPriorityList,
        slotIdentifier,
  ...
  return (
    <div>
      <select
        ...
        onChange={(e) => setPrioritySelection(e.target.value as "high" | "low")}
      >
              ...
              {generateTimeSlots(dayOfWeek).map((slot) => {
                ...
                return (
                  <button
                    ...
                    style={{
                      ...timeSlotButtonStyle,
                      backgroundColor: selectedSlots.highPriority.includes(
                        slotIdentifier,
                      )
                        ? "#029a02"
                        : selectedSlots.lowPriority.includes(slotIdentifier)
                          ? "#e5e82a"
                          : "",
                    }}
                    onMouseDown={(e) =>
                      handleMouseDown(e, slot.time, dayOfWeek)
                    }
